...
import { GetUserDataFromServer, SendLogoutRequest } from '../server/account.jsx';
...
function Account() {
    ...
    const { user, setUser, nextPath } = useMainContext();
    ...
    async function onEdit(event) {
        ...
        const [success, userData, errorMessage] = await SendEditUserRequest(firstName, surname, email, address, number, password, formData.get("password"))
        if (!success) {
        ...
        <br />
            <div className="w-100 d-flex justify-content-center">
                <form onSubmit={onEdit} className="loginbox text-align p-5">
                    <h2 className='headerbold'>Change User Details</h2>
                    ...
                    <div className={"d-flex mt-3 gap-2" + (user?.isAdmin == true ? ' justify-content-between' : ' justify-content-center')}>
                        <input className="mt-3 submitbox2" type={'submit'} value={'Confirm Changes'} name={"confirmchanges"} />
                        <input className="mt-3 submitbox2" type={'button'} value={'Log out'} name={"logout"} onClick={() => startLogout()}/>
                        {user?.isAdmin == true && <Link to={'/admin'} className="mt-3 submitbox2" type={'submit'} value={'Admin'} name={"admin"}>Admin</Link>}
                    </div>
